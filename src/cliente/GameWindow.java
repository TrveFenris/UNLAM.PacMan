...
public class GameWindow extends JFrame {
	...
	private void mensajeSalida() {
		int option = JOptionPane.showConfirmDialog(this, "Esta seguro que quiere salir?", "Saliendo del juego", JOptionPane.YES_NO_OPTION);
	...
	private void handleKeyPress(KeyEvent key) {
		if (key.getKeyCode() == KeyEvent.VK_ESCAPE) {
			...
			if (key.getKeyCode() == controles[ARRIBA]) {
				...
				if (key.getKeyCode() == controles[ABAJO]) {
					...
					if (key.getKeyCode() == controles[IZQUIERDA]) {
						...
						if (key.getKeyCode() == controles[DERECHA]) {
				...
				if (p != null) {
					switch (p.getTipo()) {
						...
						case UNIRSE_PARTIDA:
							break; // No deberia recibirse
						default:
							System.out.println("GameWindow: Paquete Desconocido.");
							break;
					}
				}
			}
